...
def view_complaint():
	...
	comp_num = sys.argv[2]
	try:
		cur.execute('''SELECT cpf, from_deptt, for_deptt, content, comp_time,\
			comp_num, created_on, approved_on, under_process_on, resolved_on\
...
def view_complaint_status():
    ...
    try:
    	...
    	for p in fetch_num:
    		cur.execute('''SELECT cpf, content, created_on, approved_on,\
    			under_process_on,resolved_on from complaints join status on\
    			complaints.num = status.comp_num AND comp_num = ?''',(p[0],))
...
def comp_for_department():
	...
	try:
	 	cur.execute('''SELECT cpf, content, created_on, approved_on, under_process_on,\
			resolved_on FROM complaints join status on for_deptt =? and complaints.num=\
			status.comp_num''', (for_dept,))
	 	db_data = cur.fetchall()
	 	result = dict()
	 	n = 1
	 	s = ''
...
def comp_from_department():
	...
	try:
		cur.execute('''SELECT cpf, content,created_on, approved_on,\
			under_process_on,resolved_on from complaints join status\
			on from_deptt =? and complaints.num=status.comp_num''',\
	 		(from_dept,))
		db_data = cur.fetchall()
		result = dict()
		n = 1
